...
// Note: Thfind `pwd` -name .htaccessommand will show you flags for other unnecessary 3rdparty files. Check only the flags for the OpenPose
...
// Producer
DEFINE_string(image_path,               "/Users/npahucki/Projects/Edmodo/gestures/images/training/NONE/4.jpg",     "Process the desired image.");
...
// OpenPose Rendering
DEFINE_bool(disable_display,            false,          "If enabled, it will not show a window, and only output the distances to the console");
...
int processInputs()
{
    try
    {
        if (!FLAGS_disable_display) {
            op::log("Starting Conversion...", op::Priority::High);
        }
        ...
        op::FrameDisplayer frameDisplayer{"Edmodo OpenPose Point Extractor", outputSize};

        UpperBodyRaisedHandPredictor predictor{FLAGS_model_folder};
        ...
        std::cout << keypointsAsNumpyTextString(poseKeypoints, {0,1,2,3,4,5,6,7}) << std::endl;


        const ArmRaised prediction = predictor.predictRasiedArm(poseKeypoints, 0);
        switch(prediction) {
            std::cerr << "PREDICTION: ";
            case ArmRaised::NONE:
                std::cerr << "No Arm Raised" << std::endl;
                break;
            case ArmRaised::RIGHT:
                std::cerr << "Right Arm Raised" << std::endl;
                break;
            case ArmRaised::LEFT:
                std::cerr << "Left Arm Raised" << std::endl;
                break;
            default:
